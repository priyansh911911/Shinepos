    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-gray-800 rounded-lg overflow-hidden max-h-[600px] overflow-y-auto">
            {filteredCustomers.map((customer) => (
              <div key={customer._id} onClick={() => fetchOrderHistory(customer._id)}
                className={`p-4 border-b border-gray-700 cursor-pointer hover:bg-gray-700 ${selectedCustomer?._id === customer._id ? 'bg-gray-700' : ''}`}>
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-green-400 text-sm">₹{customer.totalSpent || 0}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div>
          {selectedCustomer ? (
            <>
              <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-4 rounded-lg mb-4">
                <h3 className="text-white text-xl font-bold">{selectedCustomer.name}</h3>
                <p className="text-blue-200">{selectedCustomer.phone}</p>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  ...
                  <div>
                    ...
                    <p className="text-white text-2xl font-bold">₹{selectedCustomer.totalSpent || 0}</p>
                  </div>
                </div>
              </div>

              <div className="bg-gray-800 rounded-lg overflow-hidden max-h-[500px] overflow-y-auto">
                {orders.length > 0 ? orders.map((order) => (
                  <div key={order._id} className="p-4 border-b border-gray-700">
                    ...
                    <div className="mt-2">
                      {order.items?.map((item, idx) => (
                        <p key={idx} className="text-gray-400 text-sm">• {item.name} x{item.quantity}</p>
                      ))}
                    </div>
                  </div>
                )) : (
